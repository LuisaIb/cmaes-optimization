...
ggplot(data=df,aes(x=x.1,y=x.2,z=y)) +
  ...
  scale_fill_viridis(option="A",discrete = T)
```
Implementierung nach Doku o. Ä. - Luisa
...
Versuch mit mehrfacher Ausführung

```{r}
cma_es <- function (x = NULL, fun, lower, upper, 
                                 control = list(), ...) {
  #con <- list(funEvals = 1500) # default limit on function evaluations
  #con[names(control)] <- control
  #control <- con  
...
### Alternative example, using a different algorithm instead of random search
  result <- cmaes::cma_es(par = runif(length(lower),lower,upper),
                          fn=fun,lower = lower, upper=upper, ..., control = list(
                            funEvals = c(1000, 1500, 2000), 
                            maxIter = c(200, 500, 800), 
                            insigma = c(1, 2, 3, 4), 
...
for(i in 0:30){
  ...
  act_res <- c(model$like, model$Theta[1], model$Theta[2], model$Lambda)
  resultlist <- rbind(resultlist, act_res)
...
best_result <- resultlist[which.min(resultlist[,1]),]
best_result
